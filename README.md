...
### package-archives.sh (validate_structure)

Archive validation is folded into package-archives.sh's `validate_structure` function, which runs before each archive is created: verifies the presence of critical binaries, checks directory structure, and validates ELF interpreter paths for Android compatibility.
...
```bash
chmod +x test/test-workflow-end-to-end.sh
...
3. Run validation again:
   ```bash
   VERSION=1.0.0 ./scripts/package-archives.sh --version 1.0.0 --mode static --arch arm64-v8a
   ```
